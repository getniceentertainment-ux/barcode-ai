...
export default function MatrixController() {
  const { hasAccess, activeRoom, setActiveRoom, userSession, clearMatrix, audioData, isFinalized, playbackMode, setPlaybackMode, radioTrack, setRadioTrack } = useMatrixStore();
  ...
  const toggleRadioMode = () => {
    if (playbackMode === 'radio') {
      ...
      if (!radioTrack) {
        // Fallback tune-in if they hit the radio button before ever visiting Room 09
        setRadioTrack({ url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3", title: "GETNICE FM LIVE", artist: "SYNDICATE", score: 99 });
      }
      ...
      <div className="fixed bottom-0 left-0 right-0 h-24 bg-[#0a0a0a] border-t border-[#222] z-50 flex items-center px-6 lg:px-10 justify-between shadow-[0_-10px_30px_rgba(0,0,0,0.8)]">
        ...
        <div className="w-1/4 flex items-center gap-4">
          <button 
            ...
            title={playbackMode === 'radio' ? "Return to Studio Session" : "Tune into GetNice FM"}
            className={`w-12 h-12 flex items-center justify-center transition-all cursor-pointer hover:border-[#E60000] border ${playbackMode === 'radio' ? 'bg-[#110000] border-[#E60000] shadow-[0_0_15px_rgba(230,0,0,0.4)]' : 'bg-black border-[#333]'}`}
          >
            <Radio size={20} className={playbackMode === 'radio' ? "text-[#E60000] animate-pulse" : "text-[#555]"} />
          </button>
          ...
          <div className="overflow-hidden">
            ...
            <p className={`font-mono text-[9px] uppercase tracking-widest ${playbackMode === 'radio' ? 'text-[#E60000]' : 'text-[#555]'}`}>
              {playbackMode === 'radio' && radioTrack ? `SCORE: ${radioTrack.score} // ID: ${radioTrack.artist}` : (audioData?.bpm ? `${Math.round(audioData.bpm)} BPM` : 'AWAITING DSP')}
            </p>
          </div>
        </div>
