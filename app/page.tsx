...
export default function MatrixController() {
  ...
  // Automatically detects and destroys corrupted legacy sessions stuck in browser memory
  useEffect(() => {
    if (store.hasAccess && store.userSession && !store.userSession.stageName) {
      console.warn("Corrupted legacy session detected. Initiating memory purge...");
      localStorage.removeItem('barcode-matrix-storage'); // Assassinate the ghost cache
      store.clearMatrix();
      supabase.auth.signOut().then(() => {
        window.location.href = '/'; // Hard reboot
      });
    }
  }, [store.hasAccess, store.userSession, store]);
  ...
  const handleDisconnect = async () => {
    ...
    localStorage.removeItem('barcode-matrix-storage'); // Guarantee clean disconnect
    store.clearMatrix();
        ...
        <div className="p-8 border-b border-[#111]">
          ...
          {store.userSession && (
            <div className="mt-4 p-3 bg-[#050505] border border-[#222]">
              ...
              <Link href={`/${encodeURIComponent(store.userSession.stageName || "Artist")}`} className="mt-3 flex items-center justify-center gap-2 w-full bg-[#111] border border-[#333] hover:border-[#E60000] hover:text-[#E60000] text-[#888] py-1.5 text-[9px] font-mono uppercase tracking-widest transition-colors">
      ...
      <main className="flex-1 relative flex flex-col bg-black overflow-hidden">
        ...
        <div className="h-14 border-b border-[#111] bg-black/80 backdrop-blur-md flex items-center justify-between px-6 lg:px-10 z-10 shrink-0">
           ...
           <div className="flex items-center gap-4 sm:gap-6 ml-auto">
             ...
             {store.userSession && (
               <Link href={`/${encodeURIComponent(store.userSession.stageName || "Artist")}`} className="md:hidden text-[#555] hover:text-white ml-4">
                 <User size={14} />
               </Link>
             )}
