...
export default function MatrixController() {
  const { 
    ...
    audioData, isProjectFinalized, playbackMode, setPlaybackMode, "use client";
...
export default function MatrixController() {
  ...
  const isRoomLockedForTier = (roomId: string) => {
    ...
    // Free Loaders can only access Creation (R01-R04) + Radio/Social
    const freeAllowed = ["01", "02", "03", "04", "09", "10"];
    // Artists can access everything except Enterprise Social Tools (R10)
  ...
  // --- PERSISTENT CREDIT SYNC ---
  // If a generation happens in another component, we want to ensure 
  // the userSession in the store stays updated with the DB.
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { 
        ...
        // Update local store immediately when DB changes (Real-time sync)
            ...
            {rooms.map((room) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-5 py-4 text-left transition-all rounded-lg group relative ${activeRoom === room.id ? "bg-[#E60000] text-white" : "text-[#444] hover:bg-[#0a0a0a] hover:text-white"}`}
                >
      ...
      <main className="flex-1 relative flex flex-col bg-black overflow-hidden">
        ...
        <div className="h-14 border-b border-[#111] bg-black/80 backdrop-blur-md flex items-center justify-between px-10 z-10 shrink-0">
           ...
           <div className="flex items-center gap-6 ml-auto">
             {userSession && (
               <div className="flex items-center gap-2 bg-[#110000] border border-[#330000] pl-3 pr-1 py-1 rounded-full">
                 <Zap size={12} className={userSession.creditsRemaining === 0 ? "text-[#555]" : "text-yellow-500"} />
