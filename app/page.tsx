...
export default function MatrixController() {
  ...
  const toggleRadioMode = () => {
    ...
    // Hard reset of playback state when switching modes
    setIsPlaying(false);
    if (audioRef.current) {
    ...
    <div className="flex h-screen bg-[#050505] text-white overflow-hidden selection:bg-[#E60000] pb-24">
      ...
      <main className="flex-1 relative flex flex-col bg-black overflow-hidden">
        ...
        <div className="h-14 border-b border-[#111] bg-black/80 backdrop-blur-md flex items-center justify-between px-6 md:px-10 z-10 shrink-0">
           ...
           <div className="flex items-center gap-4 border-l border-[#222] pl-4 sm:pl-6 ml-auto">
               <div className={`w-1.5 h-1.5 rounded-full ${isProjectFinalized ? 'bg-yellow-500' : 'bg-[#E60000] animate-pulse'}`} />
               <span className={`text-[8px] uppercase font-bold tracking-widest ${isProjectFinalized ? 'text-yellow-500' : 'text-[#E60000]'}`}>
      ...
      {/* GLOBAL AUDIO PLAYER (Restored features) */}
      <div className="fixed bottom-0 left-0 right-0 h-24 bg-[#0a0a0a] border-t border-[#222] z-50 flex items-center px-4 md:px-10 justify-between shadow-[0_-10px_30px_rgba(0,0,0,0.8)]">
        ...
        <div className="w-1/3 md:w-1/4 flex items-center gap-2 md:gap-4">
          <button 
            ...
            className={`w-10 h-10 md:w-12 md:h-12 flex items-center justify-center transition-all border ${playbackMode === 'radio' ? 'bg-[#110000] border-[#E60000] shadow-[0_0_15px_rgba(230,0,0,0.4)]' : 'bg-black border-[#333]'}`}
          >
            <Radio size={18} className={playbackMode === 'radio' ? "text-[#E60000] animate-pulse" : "text-[#555]"} />
          </button>
          ...
          <div className="overflow-hidden hidden sm:block">
            ...
            <p className={`font-mono text-[8px] md:text-[9px] uppercase tracking-widest truncate ${playbackMode === 'radio' ? 'text-[#E60000]' : 'text-[#555]'}`}>
              {playbackMode === 'radio' && radioTrack ? `SCORE: ${radioTrack.score}` : (audioData?.bpm ? `${Math.round(audioData.bpm)} BPM` : 'AWAITING DSP')}
            </p>
          </div>
        </div>
        ...
        <div className="flex-1 max-w-2xl flex flex-col items-center justify-center px-4">
          <div className="flex items-center gap-4 md:gap-6 mb-2">
            ...
            <button 
              onClick={togglePlay} 
              disabled={playbackMode === 'session' && !audioData?.url}
        ...
        <div className="w-1/3 md:w-1/4 flex justify-end items-center gap-3">
          ...
          <input 
            ...
            className="w-16 md:w-24 h-1.5 bg-[#222] appearance-none cursor-pointer rounded-full accent-[#E60000] [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:bg-[#E60000] [&::-webkit-slider-thumb]:rounded-full"
          />
        </div>
      </div>
    </div>
