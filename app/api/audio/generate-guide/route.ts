...
export async function POST(req: Request) {
  try {
    ...
    // 🚨 SWAGGER INJECTION: Keep it to a maximum of 3 core tags. 
    // Orpheus mumbles if you overload the pre-prompt with too many brackets.
    let swaggerTag = "[rap] [confident]";

    // Clean concatenation. Remove undefined semanticTags gracefully.
    const steerableLyrics = `${swaggerTag} ${pacingTag} ${semanticTags || ""} ${lyrics}`.replace(/\s+/g, ' ').trim();
