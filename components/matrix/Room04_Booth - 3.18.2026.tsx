...
export default function Room04_Booth() {
  ...
  // 🚨 THE TELEPROMPTER TIMELINE SYNC FIX 🚨
  useEffect(() => {
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-8 pb-12 text-gray-300 font-mono text-sm leading-loose scroll-smooth">
          {lyricLines.length > 0 ? (
            lyricLines.map((line, i) => {
              ...
              return (
                <div key={i} className={`
                    ${line.isHeader ? 'text-[#E60000] font-bold mt-8 mb-2 tracking-widest text-xs' : 'mb-2 transition-all duration-300 flex items-start gap-3'}
                    ${isActive ? 'text-white text-lg font-bold bg-[#E60000]/20 py-1 px-3 border-l-2 border-[#E60000] transform translate-x-2' : ''}
                  `}>
                  {!line.isHeader && line.timestamp && (
                    <span className={`font-mono text-[9px] mt-1.5 shrink-0 ${isActive ? 'text-[#E60000]' : 'text-[#555]'}`}>{line.timestamp}</span>
                  )}
