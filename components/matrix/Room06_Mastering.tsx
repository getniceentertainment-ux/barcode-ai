...
function audioBufferToWav(buffer: AudioBuffer) {
  ...
  while(offset < buffer.length) { for(i = 0; i < numOfChan; i++) { sample = Math.max(-1, Math.min(1, channels[i][offset])); sample = (0.5 + sample < 0 ? sample * 32768 : sample * 32767) | 0; view.setInt16(pos, sample, true); pos += 2; } offset++; }
...
export default function Room06_Mastering() {
  ...
  const handleMastering = async () => {
    ...
    try {
      ...
      if(addToast) addToast("Master secured in Cloud Ledger.", "success");
      setStatus("success");
  ...
  const handleArtifactExport = async () => {
    ...
    try {
      ...
      zip.file(`${trackName}_MASTER.wav`, finalMaster.blob);
      const doc = new jsPDF(); doc.text(generatedLyrics || "", 15, 20);
      zip.file(`${trackName}_LYRICS.pdf`, doc.output('blob'));
      const zipBlob = await zip.generateAsync({ type: "blob" });
      const a = document.createElement("a"); a.href = URL.createObjectURL(zipBlob); a.download = `${trackName}_STUDIO_ZIP.zip`;
      document.body.appendChild(a); a.click(); document.body.removeChild(a);
      if(addToast) addToast("ZIP Artifact Downloaded.", "success");
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center max-w-4xl mx-auto animate-in fade-in duration-500">
      <div className="text-center mb-12">
        <h2 className="font-oswald text-5xl uppercase tracking-widest mb-4 font-bold text-white">Mastering Suite</h2>
        <p className="font-mono text-xs text-[#555] uppercase tracking-[0.2em]">Lossless Normalization to -14 LUFS</p>
      </div>
          ...
          <div className="w-full">
            ...
            {status === "success" && (
              <div className="flex flex-col items-center animate-in zoom-in duration-500 w-full space-y-4">
                <div className="w-full bg-[#0a0a0a] border border-green-500/30 p-6 flex justify-between items-center group shadow-xl">
                  ...
                  <button onClick={handleArtifactExport} disabled={isZipping} className="bg-white text-black hover:bg-[#E60000] hover:text-white p-4 transition-all">{isZipping ? <Loader2 size={24} className="animate-spin" /> : <FileArchive size={24} />}</button>
                </div>
