...
import { Sliders, CheckCircle2, Activity, ArrowRight, AudioWaveform, Disc3, Download, RefreshCw } from "lucide-react";
...
export default function Room06_Mastering() {
  const { audioData, vocalStems, setActiveRoom, addToast, finalMaster, setFinalMaster } = useMatrixStore();
  
  const [lufs, setLufs] = useState(-14); 
  
  // NEW: Initialize status and URL from the Global Matrix Store
  ...
  const handleMastering = async () => {
    ...
    try {
      ...
      // PERSIST TO GLOBAL STORE SO ROOM 07 CAN SEE IT
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center max-w-4xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="text-center mb-12">
        ...
        {status === "processing" && <p className="font-mono text-xs text-[#E60000] uppercase tracking-[0.2em] animate-pulse">Applying Multi-band Compression & True Peak Limiting...</p>}
      ...
      {status === "idle" && (
        <div className="w-full bg-[#050505] border border-[#222] p-10 flex flex-col items-center rounded-lg relative overflow-hidden group hover:border-[#E60000]/50 transition-all duration-500">
          ...
          <button onClick={handleMastering} className="relative z-10 w-full max-w-lg bg-[#E60000] text-white py-5 font-oswald text-xl font-bold uppercase tracking-[0.3em] hover:bg-red-700 transition-all shadow-[0_0_20px_rgba(230,0,0,0.2)] flex justify-center items-center gap-2">Initiate Final Master</button>
        </div>
      )}
        ...
        <div className="flex flex-col items-center animate-in zoom-in duration-500 w-full max-w-lg space-y-4">
          <div className="w-full bg-[#111] border border-[#333] p-6 flex justify-between items-center">
             ...
             <div className="flex gap-2">
                <button onClick={handleDownload} className="bg-green-500/10 border border-green-500/30 text-green-500 hover:bg-green-500 hover:text-black p-3 rounded transition-colors"><Download size={20} /></button>
             </div>
          </div>

          <div className="w-full flex flex-col gap-3">
            <button onClick={() => setActiveRoom("07")} className="w-full flex justify-center items-center gap-3 bg-white text-black py-5 font-oswald text-lg font-bold uppercase tracking-widest hover:bg-gray-200 transition-all shadow-[0_0_20px_rgba(255,255,255,0.2)]">Move to Distribution <ArrowRight size={20} /></button>
            <button onClick={handleReset} className="w-full border border-[#222] text-[#555] py-3 font-oswald text-xs font-bold uppercase tracking-widest hover:text-white hover:border-white transition-all flex justify-center items-center gap-2"><RefreshCw size={14} /> Re-Master Track</button>
          </div>
        </div>
      )}
