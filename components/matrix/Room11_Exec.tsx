...
export default function Room11_Exec() {
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      catch (parseErr) { throw new Error("Server Exception: The AI request timed out. Please try again."); }
  ...
  const handleAdvanceDay = async () => {
    ...
    try {
      ...
      const execRes = await fetch('/api/campaign/execute', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${session?.access_token}` },
        body: JSON.stringify({ trackId: submission.id, taskData: todayData, day: currentDay })
      });
      ...
      const { error: updateErr } = await supabase
        .from('submissions')
        .update({ campaign_day: nextDay, campaign_data: updatedCampaign })
        .eq('id', submission.id);
  ...
  const renderNoDeal = () => (
    <div className="h-full flex flex-col items-center justify-center p-8 text-center border border-dashed border-[#333] opacity-60">
      ...
      <p className="font-mono text-xs text-[#888] uppercase tracking-widest leading-relaxed max-w-lg mb-8">
        Access requires an Upstream Deal (Score 95+) or an Independent SaaS Purchase from Room 07.
      </p>
      <button onClick={() => setActiveRoom("07")} className="bg-[#111] border border-[#333] text-white px-8 py-3 text-[10px] font-bold uppercase tracking-widest hover:border-white transition-all">
        Unlock in Room 07
      </button>
    </div>
  );
  ...
  // 4. THE MASTER LOCK (Respects the Payola Trump Card)
  if (!submission || (!submission.upstream_deal_signed && !submission.exec_bypass && !submission.rollout_purchased)) {
     return renderNoDeal();
  }
  ...
  if (!campaignData) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-[#0a0a0a] border border-[#E60000]/30 relative overflow-hidden">
        ...
        <h2 className="font-oswald text-4xl uppercase tracking-widest text-white mb-4 font-bold relative z-10">
          {submission.upstream_deal_signed ? "Upstream Deal Detected" : "Strategy Unlocked"}
        </h2>
        <p className="font-mono text-[10px] text-[#aaa] uppercase tracking-widest leading-relaxed max-w-xl mb-10 relative z-10">
          ...
          Your track is secured. The Exec AI is ready to ingest your artifact and map out a strict 30-Day Marketing Framework. This will automate your deployment and generate your daily deliverables.
        </p>
        <button 
        ...
        <div className="absolute inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4 sm:p-8 animate-in fade-in">
          <div className="max-w-2xl w-full bg-[#050505] border border-[#333] p-8 shadow-[0_0_50px_rgba(230,0,0,0.15)] font-mono rounded-sm">
             ...
             <div className="space-y-4 min-h-[200px] flex flex-col justify-end">
               {execLogs.map((log, i) => (
                 <p key={i} className={`text-xs tracking-widest animate-in slide-in-from-bottom-2 fade-in ${log.includes('[FATAL]') ? 'text-red-500' : 'text-green-500'}`}>
