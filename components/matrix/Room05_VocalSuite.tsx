        ...
        <div className="h-64 bg-[#020202] border-t border-[#222] p-4 overflow-y-auto custom-scrollbar">
           ...
           {vocalStems.map(s => (
             <div key={s.id} className="flex items-center gap-2 bg-[#0a0a0a] p-2 border border-[#111] mb-1 group">
               ...
               <button onClick={() => removeVocalStem(s.id)} className="w-6 h-6 flex items-center justify-center text-[#444] hover:text-[#E60000]"><Trash2 size={10}/></button>
             </div>
           ))}
        </div>
      </div>
        ...
        <div className="relative z-10 max-w-xl mx-auto w-full flex-1 flex flex-col">
          <div className="bg-black/40 backdrop-blur-sm border border-[#222] p-8 mb-8">
            ...
            <div className="space-y-10">
              <div><div className="flex justify-between items-center mb-3"><label className="text-[10px] font-mono uppercase text-[#888]">Presence</label><span className="text-xs font-mono text-white">{presenceIntensity}%</span></div><input type="range" min="0" max="100" value={presenceIntensity} onChange={(e) => setPresenceIntensity(Number(e.target.value))} className="w-full accent-[#E60000]" /></div>
              <div><div className="flex justify-between items-center mb-3"><label className="text-[10px] font-mono uppercase text-[#888]">Reverb</label><span className="text-xs font-mono text-white">{reverbMix}%</span></div><input type="range" min="0" max="100" value={reverbMix} onChange={(e) => setReverbMix(Number(e.target.value))} className="w-full accent-[#E60000]" /></div>
            </div>
          </div>
