  ...
  const handleRecordCadence = async () => {
    try {
      ...
      if (audioData?.bpm) {
        ...
        else if (audioData.bpm >= 115) predictedId = "melodic_trap";
    ...
    <div className="h-full flex flex-col items-center justify-center max-w-5xl mx-auto animate-in fade-in py-10 overflow-y-auto custom-scrollbar">
      ...
      {status === "analyzing" && (
        <div className="flex flex-col items-center">
          ...
          <p className="font-mono text-[10px] text-[#E60000] uppercase animate-pulse">Running DSP Vibe Extraction...</p>
        </div>
      )}
      ...
      {status === "success" && (
        <div className="w-full animate-in zoom-in max-w-2xl mx-auto">
          ...
          <button onClick={() => setActiveRoom("03")} className="w-full bg-white text-black py-5 font-oswald font-bold text-lg uppercase tracking-[0.2em] hover:bg-[#E60000] hover:text-white transition-colors flex items-center justify-center gap-3">
