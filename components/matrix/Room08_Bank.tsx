...
export default function Room08_Bank() {
  ...
  const fetchLedger = async () => {
    ...
    if (data) {
      setRoyaltyBalance(Number(data.wallet_balance) || 0);
      setMarketingCredits(Number(data.marketing_credits) || 0);
    }
  };
  ...
  const handleAcceptDeal = async () => {
    // SECURE ROUTING: Move $1000 into the LOCKED marketing bucket
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        if (data.error === "Stripe Connect account not linked.") setNeedsOnboarding(true);
    ...
    <div className="h-full flex flex-col p-8 lg:p-12 animate-in fade-in duration-500 overflow-y-auto custom-scrollbar relative">
      ...
      <div className="max-w-6xl mx-auto w-full">
          ...
          <div className="animate-in fade-in">
            ...
            {status === "standard_payout" && (
              <div className="bg-[#050505] border border-[#222] p-16 flex flex-col items-center justify-center text-center rounded-lg animate-in zoom-in h-[300px]">
                ...
                <button onClick={() => setActiveRoom("09")} className="bg-white text-black py-4 px-8 font-oswald text-sm font-bold uppercase tracking-widest hover:bg-[#E60000] hover:text-white transition-all flex items-center gap-3">Enter Radio <ArrowRight size={16} /></button>
              </div>
            )}
            ...
            {status === "distributed" && (
              <div className="bg-[#050505] border border-green-500/30 p-16 flex flex-col items-center justify-center text-center rounded-lg animate-in zoom-in">
                ...
                <div className="bg-black border border-[#222] p-6 mb-8 w-full max-w-sm flex items-center justify-between">
                   <div className="text-left"><p className="font-mono text-[9px] text-[#888] uppercase mb-1">GetNice Wallet Balance</p><p className="font-oswald text-2xl font-bold text-green-500">$1,000.00 <span className="text-sm">CRD</span></p></div>
                   <Coins size={32} className="text-green-500" />
                </div>
                <button onClick={() => setActiveRoom("09")} className="bg-white text-black px-12 py-4 font-oswald text-lg font-bold uppercase hover:bg-[#E60000] hover:text-white transition-all flex items-center gap-3">Enter Radio <ArrowRight size={20} /></button>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === "vault" && (
          <div className="animate-in fade-in">
            {isLoadingVault ? (
              <div className="flex flex-col items-center justify-center py-20 opacity-50"><Loader2 size={48} className="animate-spin text-[#E60000]" /></div>
            ) : vaultTracks.length === 0 ? (
              <div className="text-center py-20 border border-dashed border-[#222] bg-[#050505]"><Archive size={48} className="mx-auto text-[#333] mb-4"/><p className="font-oswald text-xl uppercase text-[#555]">Vault Empty</p></div>
            ) : (
                ...
                {vaultTracks.map((track) => (
                  <div key={track.id} className="bg-black border border-[#222] hover:border-[#E60000] transition-colors group flex flex-col h-full">
                    <div className="aspect-square bg-[#111] relative overflow-hidden">
                      {track.cover_url ? <img src={track.cover_url} alt="Cover" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" /> : <Archive size={64} className="m-auto opacity-10" />}
                      <div className="absolute top-2 right-2 bg-black/80 border border-[#333] px-2 py-1 text-[9px] font-mono text-white uppercase">Score: {track.hit_score}</div>
                    </div>
                    <div className="p-4 flex-1 flex flex-col justify-between">
                      <h3 className="font-oswald text-lg uppercase text-white truncate mb-1">{track.title}</h3>
                      <div className="flex flex-col gap-2 mt-4">
                        <a href={track.audio_url} target="_blank" className="w-full flex items-center justify-center gap-2 bg-[#111] border border-[#222] text-white py-3 text-[10px] font-bold uppercase hover:bg-[#222]"><Download size={14} /> Download WAV</a>
                        {track.exec_rollout ? (
                          <button onClick={() => setActiveRollout(track.exec_rollout)} className="w-full flex items-center justify-center gap-2 bg-[#E60000] text-white py-3 text-[10px] font-bold uppercase hover:bg-red-700"><Calendar size={14} /> View Exec Rollout</button>
                        ) : (
                          <button onClick={() => handlePurchaseRollout(track)} disabled={isGeneratingRollout === track.id} className="w-full flex items-center justify-center gap-2 bg-black border border-[#333] text-yellow-500 py-3 text-[10px] font-bold uppercase hover:border-yellow-500">
                             {isGeneratingRollout === track.id ? <Loader2 size={14} className="animate-spin" /> : <Zap size={14} />} Acquire Rollout ($14.99)
                          </button>
                        )}
                      </div>
                    </div>
      ...
      {activeRollout && (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 md:p-10 animate-in fade-in">
           <div className="bg-[#050505] border border-[#E60000] w-full max-w-4xl h-full md:max-h-[85vh] flex flex-col relative shadow-[0_0_50px_rgba(230,0,0,0.3)]">
              <button onClick={() => setActiveRollout(null)} className="absolute top-6 right-6 text-[#888] hover:text-white transition-colors z-10"><X size={28} /></button>
              <div className="p-8 border-b border-[#222] bg-[#0a0a0a] shrink-0"><h2 className="font-oswald text-3xl uppercase tracking-widest font-bold text-[#E60000] flex items-center gap-3"><Calendar size={28} /> Exec Rollout Strategy</h2></div>
              <div className="p-8 overflow-y-auto custom-scrollbar font-mono text-sm text-gray-300 leading-loose whitespace-pre-wrap">{activeRollout}</div>
           </div>
        </div>
      )}
    </div>
