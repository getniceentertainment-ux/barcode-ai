...
export default function Room04_Booth() {
  ...
  useEffect(() => {
    if (waveformRef.current && audioData?.url && !wavesurferRef.current) {
      ...
      wavesurferRef.current.on('audioprocess', (time) => {
        ...
        // 🚨 THE BROWSER DRIFT FIX 🚨
        // Browsers notoriously let multiple <audio> tags drift out of sync over time.
        // This loop forces all vocal stems to mathematically lock to the Instrumental playhead!
        vocalStems.forEach(stem => {
          ...
          if (el && !el.paused) {
            ...
            // If the audio tag drifts more than 100ms from the beat, snap it back into place
            if (drift > 0.1) {
              el.currentTime = time;
            }
          }
  ...
  useEffect(() => {
    ...
    const processSection = () => {
      ...
      currentSectionLines.forEach(lineText => {
         ...
         parsed.push({ text: lineText, startTime: timeAcc, endTime: timeAcc + lineDuration, isHeader: false });
  ...
  const startHardwareRecording = async () => {
    try {
      ...
      // LOWERED PUNCH-IN LATENCY: Better for fast wired mics
            ...
            {vocalStems.map(stem => (
              <div key={stem.id} className={`flex justify-between items-center bg-[#111] p-3 border group hover:border-[#E60000]/50 transition-colors ${mutedStems.has(stem.id) ? 'border-[#330000] opacity-60' : 'border-[#333]'}`}>
                ...
                <div className="flex items-center gap-4">
                   ...
                   <button onClick={() => removeVocalStem(stem.id)} className="text-[#555] hover:text-[#E60000] transition-colors">
                     <Trash2 size={16} />
                   </button>
                </div>
              </div>
