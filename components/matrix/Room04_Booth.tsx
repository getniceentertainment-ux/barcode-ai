...
function chunkWordForVisuals(word: string): string[] {
  ...
  const pre = match[1]; const alpha = match[2]; const post = match[3];
  ...
  for (let i = 0; i < alpha.length; i++) {
    ...
    if (isVowel && !nextIsVowel && i + 2 < alpha.length) {
      ...
      if (/[aeiouy]/i.test(remaining)) { chunks.push(currentChunk); currentChunk = ""; i++; }
    }
  }
  if (currentChunk) chunks.push(currentChunk);
  if (chunks.length > 0) { chunks[0] = pre + chunks[0]; chunks[chunks.length - 1] = chunks[chunks.length - 1] + post; }
  return chunks.filter(c => c.length > 0);
}

function trimTTSBuffer(audioCtx: any, buffer: AudioBuffer): AudioBuffer {
  let startOffset = 0; let endOffset = buffer.length; const threshold = 0.03; 
  for (let c = 0; c < buffer.numberOfChannels; c++) {
    const data = buffer.getChannelData(c);
    let firstSignal = 0; let lastSignal = data.length;
    for (let i = 0; i < data.length; i++) { if (Math.abs(data[i]) > threshold) { firstSignal = i; break; } }
    for (let i = data.length - 1; i >= 0; i--) { if (Math.abs(data[i]) > threshold) { lastSignal = i; break; } }
    const pad = Math.floor(buffer.sampleRate * 0.08); 
    if (c === 0) { startOffset = Math.max(0, firstSignal - pad); endOffset = Math.min(data.length, lastSignal + pad); } 
    else { startOffset = Math.min(startOffset, Math.max(0, firstSignal - pad)); endOffset = Math.max(endOffset, Math.min(data.length, lastSignal + pad)); }
  }
  ...
  for (let c = 0; c < buffer.numberOfChannels; c++) trimmed.copyToChannel(buffer.getChannelData(c).subarray(startOffset, endOffset), c);
  return trimmed;
}

function audioBufferToWavBlob(buffer: AudioBuffer): Blob {
  const numChannels = buffer.numberOfChannels; const sampleRate = buffer.sampleRate; const format = 1; const bitDepth = 16;
  const bytesPerSample = bitDepth / 8; const blockAlign = numChannels * bytesPerSample;
  const byteRate = sampleRate * blockAlign; const dataSize = buffer.length * blockAlign;
  ...
  view.setUint32(28, byteRate, true); view.setUint16(32, blockAlign, true); view.setUint16(34, bitDepth, true); writeString(view, 36, 'data'); view.setUint32(40, dataSize, true);
...
async function trimAudioBlob(originalBlob: Blob, startSec: number, endSec: number): Promise<Blob> {
  ...
  const audioBuffer = await new Promise<AudioBuffer>((resolve, reject) => { audioContext.decodeAudioData(arrayBuffer, resolve, (err) => reject(new Error("Unable to decode audio format."))); });
  const sampleRate = audioBuffer.sampleRate; const startOffset = Math.floor(startSec * sampleRate); const endOffset = Math.floor(endSec * sampleRate);
  ...
  for (let channel = 0; channel < audioBuffer.numberOfChannels; channel++) {
    const channelData = audioBuffer.getChannelData(channel); const trimmedData = trimmedBuffer.getChannelData(channel);
...
function encodeWAV(samples: Float32Array, sampleRate: number) {
  const numChannels = 1; const format = 1; const bitDepth = 16; const bytesPerSample = bitDepth / 8;
  const blockAlign = numChannels * bytesPerSample; const byteRate = sampleRate * blockAlign; const dataSize = samples.length * blockAlign;
  ...
  view.setUint32(28, byteRate, true); view.setUint16(32, blockAlign, true); view.setUint16(34, bitDepth, true); writeString(view, 36, 'data'); view.setUint32(40, dataSize, true);
  let offset = 44;
  for (let i = 0; i < samples.length; i++) { let s = Math.max(-1, Math.min(1, samples[i])); view.setInt16(offset, s < 0 ? s * 0x8000 : s * 0x7FFF, true); offset += 2; }
...
export default function Room04_Booth() {
  ...
  const trimWaveformRef = useRef<HTMLDivElement>(null); const trimWavesurferRef = useRef<WaveSurfer | null>(null);
  const audioCtxRef = useRef<AudioContext | null>(null); const stemBuffersRef = useRef<Map<string, AudioBuffer>>(new Map());
  const activeSourcesRef = useRef<AudioBufferSourceNode[]>([]); const waveformRef = useRef<HTMLDivElement>(null);
  const wavesurferRef = useRef<WaveSurfer | null>(null); const workletNodeRef = useRef<AudioWorkletNode | null>(null);
  const mediaStreamRef = useRef<MediaStream | null>(null); const mediaSourceRef = useRef<MediaStreamAudioSourceNode | null>(null);
  const recordedChunksRef = useRef<Float32Array[]>([]); const workletLoadedRef = useRef(false);
  const teleprompterRef = useRef<HTMLDivElement>(null); const timeDisplayRef = useRef<HTMLDivElement>(null);
  const animationFrameRef = useRef<number>(); const lastActiveLineRef = useRef<number>(-1);
  const isPlayingRef = useRef(false); const isRecordingRef = useRef(false);
  ...
  const handleDragStart = (e: React.DragEvent, lineId: string, syllableId: string, originalSlot: number) => { e.dataTransfer.setData("application/json", JSON.stringify({ lineId, syllableId, originalSlot })); };
  ...
  const handleGenerateGuide = async () => {
    if (!lyricLines || lyricLines.length === 0) { if (addToast) addToast("No valid lyrics found to generate guide.", "error"); return; }
    ...
    try {
      ...
      for (let i = 0; i < parsedLines.length; i++) {
        ...
        try {
          ...
          const res = await fetch('/api/audio/generate-guide', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lyrics: aggressiveText, bpm: preciseBpm, gender: useMatrixStore.getState().gwGender || "male", pitch: "low", style: "aggressive" }) });
          ...
          mappedWords.forEach((wObj) => {
            ...
            const source = offlineCtx.createBufferSource(); source.buffer = audioBuffer; source.playbackRate.value = 1.0; 
            const gainNode = offlineCtx.createGain();
            gainNode.gain.setValueAtTime(0, wObj.startTime); gainNode.gain.linearRampToValueAtTime(1, wObj.startTime + 0.01); 
            gainNode.gain.setValueAtTime(1, wObj.startTime + wObj.duration); gainNode.gain.linearRampToValueAtTime(0, wObj.startTime + wObj.duration + tailBleed); 
            source.connect(gainNode); gainNode.connect(offlineCtx.destination);
            source.start(wObj.startTime, ttsOffset, safeExtractDuration);
          });
        } catch (lineErr) { console.warn(`Soft-fail quantizing line ${i}:`, lineErr); }
      }
    ...
    } catch (err: any) { console.error(err); if (addToast) addToast("Guide Error: " + err.message, "error"); } finally { setIsGeneratingGuide(false); setGuideProgress(0); }
  };

  const handleUpdateTakeType = (id: string, newType: string) => { const updatedStems = vocalStems.map(stem => stem.id === id ? { ...stem, type: newType as TrackType } : stem); useMatrixStore.setState({ vocalStems: updatedStems } as any); };
  ...
  const handlePurchaseEngineering = async () => {
    ...
    try {
      ...
      if (data.url) window.location.href = data.url; else throw new Error(data.error || "Failed to route to checkout.");
    } catch (err: any) { if(addToast) addToast("Checkout failed: " + err.message, "error"); }
  };

  const handleProceedToEngineering = () => {
    if (vocalStems.length === 0) { if(addToast) addToast("You must record at least one take to enter Engineering.", "error"); return; }
    stopEverything();
    if (isFreeLoader && !hasEngToken) handlePurchaseEngineering(); else setActiveRoom("05");
  };
   ...
   const applyTrim = async () => {
    ...
    } catch (err: any) { if (addToast) addToast(err.message || "Failed to slice audio.", "error"); } finally { setIsProcessingTrim(false); }
  };
    ...
    if (!isReviewMode && teleprompterEnabled && teleprompterRef.current) {
      ...
      for (let i = 0; i < lyricLines.length; i++) {
        ...
        if (visualTime >= line.startTime && visualTime < endTime) {
          currentLineIndex = i;
          lineNode.classList.add('bg-[#E60000]/10', 'border-[#E60000]'); lineNode.classList.remove('border-transparent');
          const chunks = lineNode.querySelectorAll('.syllable-chunk');
          line.words?.forEach((wObj, wIdx) => {
            const chunkNode = chunks[wIdx] as HTMLElement; if (!chunkNode) return;
            ...
            if (visualTime >= wObj.startTime && visualTime < wObj.startTime + wObj.duration) {
              chunkNode.classList.add('text-white', 'font-bold', 'drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]'); chunkNode.classList.remove('text-[#444]', 'text-[#888]');
              if (ballNode) {
                ballNode.classList.remove('hidden');
                let progress = (visualTime - wObj.startTime) / wObj.duration; progress = Math.max(0, Math.min(1, progress)); 
                const maxBounce = Math.min(16, wObj.duration * 50); const bounceHeight = Math.sin(progress * Math.PI) * maxBounce;
                ballNode.style.transform = `translateX(-50%) translateY(-${bounceHeight}px)`;
              }
            } else if (visualTime >= wObj.startTime + wObj.duration) {
              chunkNode.classList.add('text-[#888]'); chunkNode.classList.remove('text-white', 'font-bold', 'drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]', 'text-[#444]');
              if (ballNode) { ballNode.classList.add('hidden'); ballNode.style.transform = `translateX(-50%) translateY(0px)`; }
            } else {
              chunkNode.classList.add('text-[#444]'); chunkNode.classList.remove('text-white', 'font-bold', 'drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]', 'text-[#888]');
              if (ballNode) { ballNode.classList.add('hidden'); ballNode.style.transform = `translateX(-50%) translateY(0px)`; }
            }
          });
        } else {
          lineNode.classList.remove('bg-[#E60000]/10', 'border-[#E60000]'); lineNode.classList.add('border-transparent');
          const chunks = lineNode.querySelectorAll('.syllable-chunk');
          line.words?.forEach((wObj, wIdx) => {
            const chunkNode = chunks[wIdx] as HTMLElement; if (!chunkNode) return;
            const ballNode = chunkNode.querySelector('.bouncing-ball') as HTMLElement;
            if (ballNode) { ballNode.classList.add('hidden'); ballNode.style.transform = `translateX(-50%) translateY(0px)`; }
            if (visualTime >= wObj.startTime + wObj.duration) { chunkNode.classList.add('text-[#888]'); chunkNode.classList.remove('text-white', 'font-bold', 'drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]', 'text-[#444]'); } 
            else { chunkNode.classList.add('text-[#444]'); chunkNode.classList.remove('text-white', 'font-bold', 'drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]', 'text-[#888]'); }
          });
        }
      }
  ...
  const togglePlayback = async () => {
    ...
    const willPlay = !isPlaying; setIsPlaying(willPlay); isPlayingRef.current = willPlay; 
    const playheadTime = wavesurferRef.current.getCurrentTime();

    if (willPlay) {
      const scheduleTime = audioCtxRef.current.currentTime; wavesurferRef.current.play();
      if (!animationFrameRef.current) { animationFrameRef.current = requestAnimationFrame(animationTick); }
      ...
      vocalStems.forEach(stem => {
        ...
        if (buffer) {
          const source = audioCtxRef.current!.createBufferSource(); const gainNode = audioCtxRef.current!.createGain();
          source.buffer = buffer; gainNode.gain.value = stem.volume ?? 1; source.connect(gainNode); gainNode.connect(audioCtxRef.current!.destination);
          ...
          else { const bufferOffset = playheadTime - offsetSecs; if (bufferOffset < buffer.duration) source.start(scheduleTime, bufferOffset); }
  ...
  const stopEverything = async () => {
    setIsPlaying(false); isPlayingRef.current = false; setIsRecording(false); isRecordingRef.current = false;
    if (animationFrameRef.current) { cancelAnimationFrame(animationFrameRef.current); animationFrameRef.current = undefined; }
    wavesurferRef.current?.pause(); wavesurferRef.current?.seekTo(0);
    activeSourcesRef.current.forEach(src => { try { src.stop(); src.disconnect(); } catch (e) {} }); activeSourcesRef.current = [];
    ...
    if (isRecording && workletNodeRef.current && audioCtxRef.current) {
      ...
      let offset = 0; for (let chunk of recordedChunksRef.current) { merged.set(chunk, offset); offset += chunk.length; }
  ...
  const startHardwareRecording = async () => {
    ...
    if (!isMogul) {
      if (currentCredits <= 0) { if (addToast) addToast("Insufficient Credits. Top up to record a take.", "error"); return; }
      try {
        const { data: { session } } = await supabase.auth.getSession();
        const res = await fetch('/api/ledger/consume', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${session?.access_token}` }, body: JSON.stringify({ action: 'record_take', description: 'The Booth: Hardware Vocal Take' }) });
      ...
      } catch (err) { if (addToast) addToast("Ledger Sync Error. Take aborted.", "error"); return; }
    }

    if (!audioCtxRef.current) { const AudioContextClass = window.AudioContext || (window as any).webkitAudioContext; audioCtxRef.current = new AudioContextClass(); }
    ...
    try {
      ...
      const LATENCY_OFFSET = 0.06; const currentWS_Time = wavesurferRef.current?.getCurrentTime() || 0;
      ...
      const silenceNode = audioCtxRef.current.createGain(); silenceNode.gain.value = 0; 
      workletNode.connect(silenceNode); silenceNode.connect(audioCtxRef.current.destination);
  ...
  useEffect(() => {
    ...
    loadBuffers(); return () => { isMounted = false; };
  }, [vocalStems]);

  useEffect(() => {
    if (waveformRef.current && audioData?.url && !wavesurferRef.current) {
      wavesurferRef.current = WaveSurfer.create({ container: waveformRef.current, waveColor: '#333333', progressColor: '#E60000', cursorColor: '#ffffff', barWidth: 2, barGap: 1, barRadius: 2, height: 80, normalize: true });
      ...
      wavesurferRef.current.on('ready', () => { const dur = wavesurferRef.current?.getDuration() || 0; if (dur > 0) setTrackDuration(dur); });
      wavesurferRef.current.on('seeking', () => { if (!isPlayingRef.current && !isRecordingRef.current) updateVisualsRef.current(); });
  ...
  useEffect(() => {
    ...
    rawLines.forEach(line => {
      ...
      if (trimmed.startsWith('[') && trimmed.includes(']')) {
        ...
        if (cleanLine && cleanLine !== "[Instrumental Break]") llmPools[activePoolHeader].push(cleanLine);
      }
    });
    ...
    blueprint.forEach((bp, index) => {
      ...
      let activePattern = (bp as any).patternArray?.length > 0 ? (bp as any).patternArray : activeVariations[index % activeVariations.length];

      linesForThisBlock.forEach((textLine) => {
        const rawWords = textLine.split(/\s+/).filter(w => w.length > 0);
        const mappedWords: WordMapping[] = [];
        let totalLineSteps = 0; let tempPatternIndex = 0;
        ...
        const wordChunksArray = rawWords.map(w => {
          ...
          chunks.forEach(() => {
            ...
            totalLineSteps += isNaN(stepVal) ? 2 : stepVal; tempPatternIndex++;
          });
          return chunks;
        });

        const cleanTextEnd = textLine.slice(-1);
        if (cleanTextEnd === '.') totalLineSteps += 4; else if (cleanTextEnd === ',') totalLineSteps += 1;
        ...
        wordChunksArray.forEach((chunks) => {
          chunks.forEach((chunk, cIdx) => {
            ...
            mappedWords.push({
              ...
              word: chunk.replace(/\|/g, ''), slot: mappedSlot, startTime: localWordTime, duration: chunkDuration, isWordEnd: (cIdx === chunks.length - 1)
            });
        ...
        parsed.push({ 
          id: `line-${lineIdCounter++}`, barIndex: Math.floor(currentFlowTime / secondsPerBar),
          text: textLine.replace(/\|/g, ''), originalText: textLine, startTime: currentFlowTime, lineDuration: timeForLine, 
          isHeader: false, timestamp: `(${Math.floor(currentFlowTime / 60)}:${Math.floor(currentFlowTime % 60).toString().padStart(2, '0')})`,
          words: mappedWords 
        });
  ...
  useEffect(() => {
    if (trimmingStem && trimWaveformRef.current) {
      trimWavesurferRef.current = WaveSurfer.create({ container: trimWaveformRef.current, waveColor: '#555', progressColor: '#E60000', cursorColor: '#fff', barWidth: 2, barGap: 1, height: 100, normalize: true });
      ...
      trimWavesurferRef.current.on('ready', () => { const dur = trimWavesurferRef.current?.getDuration() || 0; setTrimDuration(dur); setTrimStart(0); setTrimEnd(dur); });
    }
  ...
  return (
    ...
    <div className="flex h-full bg-[#050505] border border-[#222] rounded-lg overflow-hidden animate-in fade-in duration-500 relative flex-col">
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        {/* LEFT SIDEBAR: HUD (PROMPTER / MIDI) */}
        {/* 🚨 RESPONSIVE FIX: h-1/2 on mobile, lg:h-full on desktop */}
        <div className="w-full lg:w-5/12 h-1/2 lg:h-full border-b lg:border-b-0 lg:border-r border-[#222] bg-[#020202] flex flex-col relative shadow-[inset_-10px_0_30px_rgba(0,0,0,0.5)]">
          <div className="p-4 lg:p-8 pb-4 border-b border-[#111] flex justify-between items-center relative flex-wrap gap-4">
             <div className="flex items-center gap-4">
               ...
               <div className="flex border border-[#333] rounded overflow-hidden">
                 <button onClick={() => setIsReviewMode(false)} className={`px-3 py-1.5 text-[9px] uppercase font-mono font-bold transition-all flex items-center gap-1.5 ${!isReviewMode ? 'bg-[#E60000]/20 text-[#E60000]' : 'bg-black text-[#555] hover:text-white'}`}>
                   <ListVideo size={12} /> Prompter
                 </button>
                 <button onClick={() => setIsReviewMode(true)} className={`px-3 py-1.5 text-[9px] uppercase font-mono font-bold transition-all flex items-center gap-1.5 border-l border-[#333] ${isReviewMode ? 'bg-yellow-500/10 text-yellow-500' : 'bg-black text-[#555] hover:text-white'}`}>
                   <Crosshair size={12} /> Quantize Grid
                 </button>
               </div>
             </div>
             {!isReviewMode && (
               <button onClick={handleGenerateGuide} disabled={isGeneratingGuide || !generatedLyrics} className="bg-[#111] border border-[#333] text-[#E60000] hover:bg-white hover:text-black hover:border-white px-2.5 py-1 text-[9px] uppercase font-mono font-bold transition-all flex items-center gap-1.5 disabled:opacity-50">
          ...
          {isReviewMode ? (
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 lg:p-6 bg-[#020202]">
                <div className="mb-6 flex justify-between items-end border-b border-[#222] pb-4">
                  <div><h3 className="font-oswald text-sm uppercase tracking-widest text-white">MIDI Flow Engine</h3><p className="font-mono text-[9px] text-[#888] mt-1">Snaking Drag enabled. Shifts subsequent text.</p></div>
                  <div className="flex gap-2 text-[8px] font-mono text-[#555] uppercase font-bold"><span>Kick (1)</span> | <span>Snare (2,4)</span></div>
                </div>
                    ...
                    <div key={line.id} className="bg-black border border-[#222] rounded overflow-hidden">
                      ...
                      <div className="flex h-10 relative bg-[#0a0a0a]">
                        {Array.from({ length: 16 }).map((_, slotIndex) => {
                          ...
                          const isDownbeat = slotIndex % 4 === 0; const isSnare = slotIndex === 4 || slotIndex === 12;
                          return (
                            <div key={slotIndex} onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDrop(e, line.id, slotIndex)} className={`flex-1 border-r border-[#111] relative flex items-center justify-center ${isDownbeat ? 'bg-[#151515]' : ''} ${isSnare ? 'bg-[#2a0505]' : ''}`}>
                              {mappedSyl && (
                                <div draggable onDragStart={(e) => handleDragStart(e, line.id, mappedSyl.id, mappedSyl.slot)} className="absolute z-10 w-[90%] py-1 bg-[#E60000] text-white text-[9px] font-mono font-bold text-center rounded cursor-grab active:cursor-grabbing shadow-[0_0_10px_rgba(230,0,0,0.4)] hover:bg-red-500 overflow-hidden text-ellipsis whitespace-nowrap" title={mappedSyl.word}>
                                  {mappedSyl.word}
                                </div>
                              )}
                            </div>
            ...
            <div ref={teleprompterRef} onWheel={() => setAutoScroll(false)} onTouchMove={() => setAutoScroll(false)} className="flex-1 overflow-y-auto custom-scrollbar px-4 lg:px-8 pb-12 text-gray-300 font-mono text-sm leading-loose relative">
              {lyricLines.map((line, i) => {
                if (line.isHeader) return <p key={i} className="lyric-line-container text-[#E60000] font-bold mt-8 mb-2 tracking-widest text-xs">{line.text}</p>;
                ...
                return (
                  <div key={i} className={`lyric-line-container mb-2 py-2 px-3 border-l-2 ${isActiveLine ? 'bg-[#E60000]/10 border-[#E60000]' : 'border-transparent'} flex items-start gap-3 transition-colors duration-200`}>
                    ...
                    <span className="flex-1 leading-loose flex flex-wrap gap-y-2">
                      {(() => {
                        const wordGroups: WordMapping[][] = []; let currentGroup: WordMapping[] = [];
                        line.words?.forEach(wObj => { currentGroup.push(wObj); if (wObj.isWordEnd) { wordGroups.push(currentGroup); currentGroup = []; } });
                          ...
                          <span key={gIdx} className="inline-flex whitespace-nowrap mr-2">
                            {group.map((wObj, wIdx) => (
                              <span key={wIdx} className="syllable-chunk relative inline-block text-[#444] transition-colors duration-100">
                                <span className="bouncing-ball hidden absolute bottom-full mb-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-[#E60000] rounded-full shadow-[0_0_8px_#E60000] z-50"></span>{wObj.word}
                              </span>
                            ))}
                          </span>
                        ));
              ...
              {teleprompterEnabled && !autoScroll && (
                <div className="sticky bottom-4 w-full flex justify-center mt-8">
                  <button onClick={() => { setAutoScroll(true); if (lastActiveLineRef.current !== -1 && teleprompterRef.current) { const activeEl = teleprompterRef.current.children[lastActiveLineRef.current] as HTMLElement; if (activeEl) teleprompterRef.current.scrollTo({ top: activeEl.offsetTop - 150, behavior: 'smooth' }); } }} className="bg-[#E60000] text-white text-[9px] px-4 py-2 font-mono uppercase tracking-widest shadow-[0_0_15px_rgba(230,0,0,0.5)] rounded-full flex items-center gap-2 transition-all hover:bg-red-700 hover:scale-105"><Activity size={12} className="animate-pulse" /> Resume Sync</button>
                </div>
              )}
            </div>
          )}
        ...
        {/* 🚨 RESPONSIVE FIX: h-1/2 on mobile, lg:h-full on desktop */}
        <div className="w-full lg:flex-1 h-1/2 lg:h-full flex flex-col relative bg-black">
          <div className="h-20 lg:h-24 bg-black border-b border-[#222] flex items-center justify-between px-4 lg:px-10 relative">
            <div className="flex items-center gap-2 lg:gap-4">
              <button onClick={togglePlayback} disabled={isUploading} className="w-12 h-12 lg:w-14 lg:h-14 rounded-full border border-[#333] flex items-center justify-center bg-[#111] hover:bg-white hover:text-black transition-all disabled:opacity-50">
                {isPlaying && !isRecording ? <Pause size={20} /> : <Play size={20} className="ml-1" />}
              </button>
              <button onClick={stopEverything} disabled={isUploading} className="w-12 h-12 lg:w-14 lg:h-14 rounded-full border border-[#333] flex items-center justify-center bg-[#111] hover:bg-white hover:text-black transition-all text-[#888] disabled:opacity-50">
                <Square size={18} />
              </button>
              <button onClick={isRecording ? stopEverything : startHardwareRecording} disabled={isUploading} className={`w-12 h-12 lg:w-14 lg:h-14 rounded-full flex items-center justify-center transition-all disabled:opacity-50 ${isRecording ? 'bg-red-950 text-[#E60000] border-2 border-[#E60000] animate-pulse' : 'bg-[#111] border border-[#333] text-white hover:text-[#E60000]'}`}>
                {isUploading ? <Loader2 size={20} className="animate-spin" /> : <Mic size={20} />}
              </button>
            </div>
            
            {isUploading && (
               <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2 text-[#E60000] bg-[#110000] px-3 py-1 lg:px-4 lg:py-1.5 border border-[#E60000]/30 rounded-full animate-pulse">
                 <Activity size={14} />
                 <span className="font-mono text-[8px] lg:text-[9px] uppercase tracking-widest font-bold">Syncing...</span>
               </div>
            )}
            <div ref={timeDisplayRef} className="font-mono text-2xl lg:text-3xl font-bold tracking-widest text-[#E60000]">00:00</div>
          </div>

          <div className="p-4 lg:p-6 border-b border-[#222] bg-[#050505]">
            <div ref={waveformRef} className="w-full h-16 lg:h-20 bg-black border border-[#111] rounded-lg"></div>
          </div>

          <div className="flex-1 p-4 lg:p-6 overflow-y-auto custom-scrollbar">
            <div className="flex border-b border-[#111] mb-6 flex-wrap gap-2">
              {(["Lead", "Adlib", "Double", "Guide"] as TrackType[]).map(t => (
                <button key={t} onClick={() => setActiveTrack(t)} className={`flex-1 py-2 lg:py-3 min-w-[60px] font-oswald text-[10px] uppercase tracking-[0.2em] font-bold transition-all ${activeTrack === t ? 'bg-[#E60000] text-white' : 'text-[#444] hover:text-white hover:bg-[#0a0a0a]'}`}>{t}</button>
              ))}
            </div>
            ...
            <div className="space-y-3">
              {vocalStems.map(s => (
                <div key={s.id} className="bg-[#0a0a0a] border border-[#222] p-3 lg:p-4 rounded group transition-all">
                  {/* 🚨 RESPONSIVE FIX: flex-wrap so elements stack nicely if needed */}
                  <div className="flex justify-between items-center mb-3 flex-wrap gap-2">
                    <div className="flex items-center gap-2 sm:gap-4">
                      ...
                      <span className="font-mono text-[10px] text-[#444] hidden sm:inline">{s.id.substring(5, 12)}</span>
                    ...
                    <div className="flex items-center gap-2 sm:gap-4">
                      <div className="flex items-center gap-2 mr-2 sm:mr-4">
                        ...
                        <button onClick={() => toggleStemMute(s.id)} className={`transition-colors ${s.isMuted ? 'text-[#E60000]' : 'text-[#888] hover:text-white'}`}>{s.isMuted ? <VolumeX size={14} /> : <Volume2 size={14} />}</button>
                      </div>
                  ...
                  <div className="flex flex-col gap-4 mt-3 border-t border-[#111] pt-4">
                    <div className="flex items-center gap-2 sm:gap-4">
                      <span className="text-[8px] sm:text-[9px] font-mono text-[#555] uppercase w-12 sm:w-16 tracking-widest">Offset</span>
                    ...
                    <div className="flex items-center gap-2 sm:gap-4">
                      <span className="text-[8px] sm:text-[9px] font-mono text-[#555] uppercase w-12 sm:w-16 tracking-widest">Gain</span>
          ...
          <div className="h-16 bg-black border-t border-[#222] flex items-center justify-between px-4 lg:px-10">
            <div className="flex items-center gap-2 text-[8px] sm:text-[10px] font-mono text-green-500 uppercase tracking-widest opacity-80">
              {vocalStems.length > 0 && <><Save size={14} className="hidden sm:block"/> Synced</>}
            </div>
            {isFreeLoader && !hasEngToken ? (
              <button onClick={handleProceedToEngineering} disabled={vocalStems.length === 0} className="flex items-center gap-2 sm:gap-3 bg-[#E60000] text-white px-4 sm:px-8 py-2 font-oswald font-bold uppercase tracking-widest text-[10px] sm:text-xs hover:bg-red-700 transition-all disabled:opacity-30">Unlock Engineering <Lock size={14} /></button>
            ) : (
              <button onClick={handleProceedToEngineering} disabled={vocalStems.length === 0 || isUploading} className="flex items-center gap-2 sm:gap-3 bg-white text-black px-4 sm:px-8 py-2 font-oswald font-bold uppercase tracking-widest text-[10px] sm:text-xs hover:bg-[#E60000] hover:text-white transition-all disabled:opacity-30 disabled:cursor-not-allowed">Engineering <ArrowRight size={16} /></button>
            )}
          </div>
        </div>
      </div>
      ...
      {trimmingStem && (
        <div className="absolute inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 lg:p-8 animate-in zoom-in duration-300">
          <div className="bg-[#050505] border border-[#E60000] rounded-lg w-full max-w-2xl p-6 lg:p-8 shadow-[0_0_50px_rgba(230,0,0,0.2)]">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-oswald text-xl lg:text-2xl uppercase tracking-widest text-[#E60000] font-bold flex items-center gap-3"><Scissors size={24} /> Slice Region</h3>
              <button onClick={() => setTrimmingStem(null)} className="text-[#555] hover:text-white"><X size={24}/></button>
            </div>
            <p className="font-mono text-[9px] lg:text-[10px] text-gray-400 uppercase tracking-widest mb-6">Drag sliders to crop dead air from the microphone take.</p>
            ...
            <div className="flex justify-between font-mono text-[9px] lg:text-[10px] text-[#888] mt-4 uppercase"><span>Start: {trimStart.toFixed(2)}s</span><span>Keep: {(trimEnd - trimStart).toFixed(2)}s</span><span>End: {trimEnd.toFixed(2)}s</span></div>
            <button onClick={applyTrim} disabled={isProcessingTrim} className="w-full mt-8 bg-[#E60000] text-white py-4 font-oswald text-base lg:text-lg font-bold uppercase tracking-widest hover:bg-red-700 transition-all flex items-center justify-center gap-3 disabled:opacity-50">{isProcessingTrim ? <Loader2 size={20} className="animate-spin" /> : <><Scissors size={20} /> Execute Destructive Slice</>}</button>
          </div>
        </div>
      )}
    </div>
